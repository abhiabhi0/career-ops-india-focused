...
async function main() {
  ...
  if (runPhase3) {
    ...
    if (total === 0) {
      ...
      // Also write raw URLs to data/search-urls.tsv for reference
      if (!dryRun && uniqueResults.length > 0) {
        const urlsFile = 'data/search-urls.tsv';
        const header = 'url\ttitle\tquery_name\n';
        const rows = uniqueResults.map(e =>
          `${e.url}\t${(e.title || '').replace(/\t/g, ' ')}\t${e.queryName}`
        ).join('\n');
        writeFileSync(urlsFile, header + rows + '\n', 'utf-8');
      }
    }
  }
